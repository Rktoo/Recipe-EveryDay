        ...
        <ul className='fixed flex flex-row gap-1 border-2 rounded-full px-10 overflow-hidden backdrop-blur-md'>
          {
            menuNavbar && menuNavbar.map(menu => {
              return (
                <li className={`my-1 px-4 py-4 ${menu.icon} bg-cover opacity-75 hover:scale-x-110 hover:z-[60] hover:opacity-100 transition-all duration-100 ease-in-out text-white rounded-full`} title={menu.nom} key={menu.nom} >
                <Link to={menu.lien} className='text-white hover:text-white '>{menu.nom}</Link>
              </li>
            )
          }
          )
